...
pub struct WaylandStopHandler {
    ...
    pub session_path: Arc<Mutex<String>>,
}

// unsafe impl Send for WaylandStopHandler {}

pub async fn close_session(session_path: String, connection: &Connection) {
    if session_path.len() == 0 {
        return;
    }
    let session_path = session_path.replace("request", "session");
    log::info!(
        "Closing session...: {:?}",
        session_path
    );
...
impl WaylandStopHandler {
    pub async fn stop(&self) {
        let session_path = self.session_path.lock().expect("Get session path").clone();
        close_session(
            session_path,
            &self.connection,
        )
...
impl WaylandRecorder {
    pub async fn new(id: &str) -> Result<(Self, WaylandStopHandler)> {
    ...
    pub async fn run(
            ...
            match msg.message_type() {
                message::Type::Error => {
                    ...
                    log::error!(
                        ...
                        s,
                        self.session_path.lock().expect("Get session path")
                    );
                }
                message::Type::Signal => {
                    let body = msg.body();
                    ...
                    if response.contains_key("session_handle") {
                        let session_path = response
                            ...
                            .clone()
                            .downcast::<String>()?;
                        self.session_path = Arc::new(Mutex::new(session_path.clone()));

                        self.handle_session(record_type, cursor_mode_type)
                            .await
                            .map_err(|e| {
                                anyhow!("{}. Session handle: {}", e, &session_path)
                            })?;
                        continue;
                    }
        ...
        log::info!("No more messages. Session path: {}", self.session_path.lock().expect("Get session path"));
        Ok(())
    }
    ...
    async fn handle_session(
        ...
        self.screen_cast_proxy
            .select_sources(
                &ObjectPath::try_from(session_path.clone())?,
                option_map,
            )
            .await?;
